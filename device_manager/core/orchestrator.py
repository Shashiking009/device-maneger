...
from core.jarvis_personality import jarvis_personality
...
class SpidyOrchestrator:
    ...
    Processes both text commands and voice inputs through a unified pipeline:
    ...
    def process_command(self, user_input: str, session_id: Optional[str] = None, use_rag: bool = True) -> SpidyResponse:
        ...
        # 0a. Conversational Pronoun & Memory Reference Resolution ("Close it" -> "Close Google Chrome")
        user_input_resolved = context_manager.resolve_reference(user_input, session_id=session_id)
        user_input_resolved = conversation_context.resolve_references(user_input_resolved)

        # 0b. Multi-Step Request Handling ("Open Chrome and Notepad")
        sub_cmds = self.router.extract_multi_step_commands(user_input_resolved)
        if len(sub_cmds) > 1:
            ...
            if len(plan.actions) > 1:
                ...
                summary_msg = f"Done, boss. Executed {len(res_plan.actions)} steps."
                ...
                conversation_context.add_turn("user", user_input, "MULTI_ACTION")
                conversation_context.add_turn("assistant", summary_msg, "MULTI_ACTION")
        ...
        if intent.name in [IntentType.MEMORY_SAVE, IntentType.MEMORY_QUERY, IntentType.MEMORY_DELETE, IntentType.MEMORY_CLEAR]:
            if intent.name == IntentType.MEMORY_CLEAR and not intent.parameters.get("confirmed", False):
                return SpidyResponse(
                    success=False,
                    intent=intent.name,
                    message="Boss, clearing memory will delete all saved preferences. Confirm with 'Yes' to proceed.",
            ...
            if intent.name == IntentType.MEMORY_CLEAR and intent.parameters.get("confirmed", False):
                ...
                return SpidyResponse(success=True, intent=intent.name, message="Cleared all saved memories, boss.")

            succ, msg, data = memory_service.process_memory_command(user_input_resolved)
            formatted_msg = jarvis_personality.format_response(intent.name.value, msg, succ)
            conversation_context.add_turn("user", user_input, intent.name.value)
            conversation_context.add_turn("assistant", formatted_msg, intent.name.value)
            return SpidyResponse(
                success=succ,
                intent=intent.name,
        ...
        if not self.permission_mgr.is_permitted(intent):
            return SpidyResponse(
                success=False,
                intent=intent.name,
                message=f"Action '{intent.name.value}' is blocked by security policy, boss."
            )
        ...
        if intent.name in self.registry.handlers:
            ...
            latency_ms = round((time.time() - start_t) * 1000, 2)
            formatted_msg = jarvis_personality.format_response(intent.name.value, resp.message, resp.success)
            resp.message = formatted_msg

            # Update Conversation Context & Session Objects
            meta = {}
            if intent.name == IntentType.OPEN_APPLICATION or intent.name == IntentType.CLOSE_APPLICATION:
                meta["app_name"] = intent.target
            elif intent.name == IntentType.OPEN_FOLDER:
                meta["folder_name"] = intent.target
            elif intent.name == IntentType.READ_FILE or intent.name == IntentType.SEARCH_FILE:
                meta["file_name"] = intent.target

            conversation_context.add_turn("user", user_input, intent.name.value, metadata=meta)
            conversation_context.add_turn("assistant", formatted_msg, intent.name.value, metadata=meta)
        ...
        if intent.name == IntentType.RAG_QUERY or (use_rag and intent.name in [IntentType.AI_QUESTION, IntentType.UNKNOWN]):
            ...
            if rag_res.sources or "couldn't find" in rag_res.answer:
                ...
                conversation_context.add_turn("user", user_input, "RAG_QUERY")
                conversation_context.add_turn("assistant", reply, "RAG_QUERY")
        ...
        # 5. Direct Local Qwen3 AI Completion with Bounded Conversation Context
        ctx_prompt = conversation_context.get_bounded_context_prompt(max_messages=4)
        prompt = (
            "You are Spidy, a privacy-first JARVIS-style local desktop AI assistant powered by Qwen3. "
            "Provide a concise, helpful answer in 2 sentences.\n"
        )
        if ctx_prompt:
            prompt += f"Recent Conversation:\n{ctx_prompt}\n"
        prompt += f"User: {user_input_resolved}\nAssistant:"

        reply, tps = self.qwen.generate_ai_response(prompt)
        latency_ms = round((time.time() - start_t) * 1000, 2)
        
        # Track AI topic in session context
        topic = conversation_context.extract_ai_topic(user_input_resolved)
        conversation_context.add_turn("user", user_input, intent.name.value, metadata={"ai_topic": topic})
        conversation_context.add_turn("assistant", reply, intent.name.value, metadata={"ai_topic": topic})
